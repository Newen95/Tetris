...
// paramètres d'entrée : pointeur sur le tableau des joueurs, pointeur sur le nombre de joueurs (= nombre de lignes du fichier)
...
void lectureScore( Joueur** tabJoueurs, int* nbLignes) {
	...
	*tabJoueurs = malloc( *nbLignes * sizeof( Joueur ) );
	...
	int numLigne = 0;
	while( fgets( ligne, 20, fichierClassement ) != NULL ) { // 20 = nombre max de caractères d'une ligne
